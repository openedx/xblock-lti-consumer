...
class TestToolKeyHandler(TestCase):
    ...
    def setUp(self):
        ...
        rsa_jwk['kid'] = self.rsa_key_id
        self.key = PyJWK.from_dict(rsa_jwk)

        rsa_public_jwk = json.loads(algo.to_jwk(key_data.public_key()))
        rsa_public_jwk['kid'] = self.rsa_key_id
        self.public_key = PyJWK.from_dict(rsa_public_jwk)
